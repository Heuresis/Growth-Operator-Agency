...
| `copy-messaging-mastery.md` | The tactical application layer: Copy Payoffs, Copy Logic (true-statement + if/then chains), Motivational vs Educational Copy, Creating Desire, Creating Rationalizations, Rapport and Openings, Objection-Handling story structures. | ~3,150w |
| `strategic-foundations.md` | The business-model layer: Competing on Economics, AIDVVA update to AIDA, Rational vs Irrational Math, Three Economics Dimensions, Intentional Business Models, Dynamic vs Static Markets, Creative vs Mechanical Markets, Need vs Want Utility, Format Innovation lifecycle. | ~2,750w |

## Related Files in Neighbour Directories

These pre-existing files are concept-thematic rather than operator-indexed, but cover Sultanic material. They continue to live in `../esoteric-marketing/` and `../offer-architecture/` and are cross-referenced where relevant.
...
| Funnel Format Architecture (time/attention economics) | `../esoteric-marketing/funnel-format-arch.md` |
| VSL Story Architecture (21-step Hero's Journey) | `../esoteric-marketing/vsl-story-architecture.md` |
| Engineering Economics of an Offer (backend) | `../esoteric-marketing/engineering-economics-offer.md` |
...
**Diagnosing a stuck campaign.** `math-psychology-framework.md` (identify which side is broken) → `7-phase-offer-architecture.md` diagnostic-checklist section → `advanced-psychological-mechanisms.md` if the issue is belief-stage mismatch.

**Writing / editing copy.** `copy-messaging-mastery.md` as primary. `advanced-psychological-mechanisms.md` (Timeline Language section specifically) for the language-pattern detail.

**Making strategic decisions.** `strategic-foundations.md` for the business-model layer. `power-with-cognitive-os.md` for the ethical / relational axis, especially when deciding whether to build a DFY vs course offering.
...
- Direct quotes from source JSON are preserved verbatim across all seven files
- Uncertain attributions are tagged `[UNVERIFIED]`
- Banned vocabulary per `spec/BANNED-VOCABULARY.md` is excluded
- Cases cited are only those explicitly present in the source playbooks
- No fabricated client names, no fabricated quotes, no fabricated case details
