...
> **Source / Lineage:** Dustin Varano (YouTube Flywheel Funnel). This file goes deeper than `reference/frameworks/youtube/7-video-types.md` on *how the types interact*. The 7-video-types reference catalogs each format; this file explains the machine they form when composed correctly.

## Why This Matters

A channel that publishes one of each of the 7 video types but does not link them together is producing content, not operating a flywheel. The Flywheel is the specific set of linking policies — which video feeds which, which carries a CTA, which routes to the VSSL, which gets re-cut into short-form — that turns a video library into a self-reinforcing machine. Most operators importing Varano copy the content types and miss the linking policies, and the system underperforms for reasons they cannot diagnose.
...
A viewer who lands on a Varano-model channel for the first time ideally traverses the following path:
...
- `reference/frameworks/dustin-varano/education-os-4-phase.md` (phase-layer on top of the Flywheel)
- `reference/frameworks/dustin-varano/business-partner-model.md` (operator engagement that runs the Flywheel)

---
*v1.0 — 2026-04-19 — Varano Flywheel composition reference.*
