...
Luna, Roberto. *Signal Theory: Optimal Intent Encoding.* 63pp.
...
`SYSTEM.md` section 1 declares a named lineage of operators and thinkers whose pre-trained representations an LLM agent should activate when entering this workspace. That list is a technical activation payload, not a recommendation list. It names practitioners in go-to-market (Sultanic, Pampa, Hormozi, Haynes, Clogg, Varano, Hogendoorn, Georgi, Brown, Benson, Gadzhi), persuasion theorists (Schwartz, Cialdini, Kahneman, Halbert), and systems thinkers (Beer, Ashby, Shannon, Wiener). Each is cited in the specific framework file that encodes their method under `reference/frameworks/` or `reference/operators/`, with a Sources section pointing at the original surface.
...
- The FIOVA agent architecture (the 82KB bible at `reference/FIOVA-AGENT-ARCHITECTURE.md`).
...
- The public-domain classical theory (Shannon, Ashby, Beer, Wiener) cited inside Signal Theory and Quality.
