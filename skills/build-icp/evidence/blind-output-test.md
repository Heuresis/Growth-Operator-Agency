...
> Per `_archive/spec/BLIND-OUTPUT-TEST.md`. This skill produces an **ICP Document**. Classification: **internal** — requires **1/3 evaluator pass** before the artifact is used as audience ground truth for downstream external-facing skills.
...
- 1 generic-LLM ICP (control — no Growth OS context)
...
- **≥ 1 of 3 Growth OS ICPs** must have ≥ 1 evaluator say `creator` or `can't tell`
- **No generic-LLM ICP** should be identified as `creator`
- Average "Would you act on this?" score ≥ 50% yes for Growth OS ICPs
