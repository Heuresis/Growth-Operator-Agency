...
> Per `_archive/spec/BLIND-OUTPUT-TEST.md`. This skill produces an **Email Sequence** (one of 8 types). Classification: **external** — requires **2/3 evaluator pass** before sending to creator's production list.
...
- 1 generic-LLM email (control — no Growth OS context)
...
- **≥ 2 of 3 Growth OS emails** must have ≥ 1 evaluator say `creator` or `can't tell`
- **No generic-LLM email** should be identified as `creator`
- Average "Would you send this?" ≥ 60% yes for Growth OS emails
